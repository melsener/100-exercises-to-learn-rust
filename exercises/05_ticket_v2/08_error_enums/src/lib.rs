...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(error) => {
            match error {
                TicketNewError::EmptyTitle(message) | TicketNewError::TooLongTitle(message) => panic!("{message}"),
                TicketNewError::EmptyDescription(_) | TicketNewError::TooLongDescription(_) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::EmptyTitle("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TooLongTitle("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::EmptyDescription("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::TooLongDescription("Description cannot be longer than 500 bytes".to_string()));
        }
